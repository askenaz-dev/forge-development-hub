...
The `installer-core` change delivered a working CLI for a 30-developer pilot, but the Git-backed registry is only browseable through whichever Git host hosts the repo, and the CLI's name (`fdh`) reflects the implementation rather than the product the org wants to invest in. To grow adoption from 30 pilot developers to the full Forge engineering org, three things must land together: (1) a modern, professional web portal where developers discover, browse, and learn to install skills; (2) Keycloak single sign-on so the portal honors Forge's existing identity provider and can carry roles into governance work later; (3) a product-grade CLI name — `fdh` (Forge Development Hub) — that the portal, docs, and Slack culture can all stand behind. Doing this as one change keeps the rename and the portal aligned so the docs and install snippets never reference a stale binary name.

## What Changes

- **BREAKING — Rename the CLI from `fdh` to `fdh`.** The repository moves from `forge/skill-installer` to `forge/fdh`; the Go module becomes `github.com/forge/fdh`; the binary is `fdh` / `fdh.exe`; the per-user config directory moves from `~/.config/fdh/` to `~/.config/fdh/`. A back-compat stub binary keeps `fdh` working for a 90-day deprecation window, printing a one-line notice and forwarding to `fdh`.
...
- Ship **local-dev infrastructure** (Docker Compose bringing up Keycloak + API + Web together) and **production deployment infrastructure** (Helm chart for Forge's existing Kubernetes cluster).
...
- **Repository rename**: `forge/skill-installer` becomes `forge/fdh`. The old repo retains a `README.md` and a stub binary release pointing to the new location for 90 days, then archived.
- **Go module rename**: every import of `github.com/forge/skill-installer/...` becomes `github.com/forge/fdh/...`. Mechanical refactor across all source files and tests.
- **Per-user config migration**: `fdh` reads from `~/.config/fdh/config.yaml` (and `~/.config/fdh/adapters.yaml`) but for the 90-day deprecation window also reads the old `~/.config/fdh/config.yaml` if the new file is absent and emits a one-line warning. A `fdh config migrate` command performs the move explicitly.
