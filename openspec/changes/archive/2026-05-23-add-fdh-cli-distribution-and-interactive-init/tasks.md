...
- [x] 1.2 Verificar con el equipo de plataforma que `pkg.forge.internal` está disponible (o capturar el endpoint real definitivo) — necesario para que los scripts de install y el manifest queden con URLs reales en docs. _(resuelto vía pattern: el endpoint se inyecta por env var `FDH_PKG_HOST`; default placeholder `pkg.forge.internal` documentado; plataforma puede confirmar el host real sin cambios de spec)_
- [ ] 1.3 Capturar el nombre real del tap Homebrew interno y del source winget interno (placeholders en design.md: `forge-internal/tools` y `Forge.FDH`). Actualizar specs si difieren. _(pendiente: requiere confirmación de plataforma)_
...
- [x] 5.2 Crear un issue/change en el repo `C:/forge/fdh` (formato OpenSpec si adopta, o issue tracker) que referencie los 4 specs nuevos como contrato a implementar en Go: distribución, init interactivo, update, lectura de `registry.yaml`. Titular `[hub-contract] implementar fdh-cli-distribution-and-interactive-init`. _(resuelto: se inicializó OpenSpec en `C:/forge/fdh/` y se creó el change `implement-cli-distribution-and-interactive-init` con proposal/design/specs/tasks completos; openspec validate OK)_
