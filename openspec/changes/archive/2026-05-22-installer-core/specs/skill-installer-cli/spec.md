...
The `config get <key>` and `config set <key> <value>` commands SHALL manage a minimal configuration surface persisted under `~/.config/fdh/config.yaml` (or the OS-appropriate equivalent on Windows). Supported keys MUST include at least `registry.url`, `defaults.scope`, and `cache.dir`. Unknown keys MUST be rejected.
