...
The CLI SHALL be distributed as a single statically-linked Go binary with no runtime dependencies on the target machine. Release artifacts MUST be produced for macOS arm64, macOS amd64, Linux arm64, Linux amd64, and Windows amd64.

#### Scenario: Binary runs without runtime dependencies

- **WHEN** a developer downloads the binary for their platform and runs `falabella-installer --version`
...
- **THEN** the release artifacts include exactly five binaries: `falabella-installer-darwin-arm64`, `falabella-installer-darwin-amd64`, `falabella-installer-linux-arm64`, `falabella-installer-linux-amd64`, `falabella-installer-windows-amd64.exe`
...
- **WHEN** a developer runs `falabella-installer install code-review-standard` on a host where all four agents are detected
...
- **WHEN** a developer runs `falabella-installer install code-review-standard --agent claude-code`
...
- **WHEN** a developer runs `falabella-installer install code-review-standard` on a host with no detectable agents
...
- **WHEN** a developer has installed two skills and runs `falabella-installer list`
...
- **WHEN** a developer runs `falabella-installer list` on a directory where a `SKILL.md` exists without a `.skill-meta.yaml` sidecar
...
- **WHEN** a developer runs `falabella-installer doctor` on a correctly configured host
...
- **WHEN** `falabella-installer doctor` runs in a directory without any project configuration and without network access
...
- **WHEN** a developer runs `falabella-installer search owasp` against a registry containing a skill named `security/owasp-review`
...
- **WHEN** a developer runs `falabella-installer search nonexistent-term`
...
The `config get <key>` and `config set <key> <value>` commands SHALL manage a minimal configuration surface persisted under `~/.config/falabella-installer/config.yaml` (or the OS-appropriate equivalent on Windows). Supported keys MUST include at least `registry.url`, `defaults.scope`, and `cache.dir`. Unknown keys MUST be rejected.

#### Scenario: Set and read back a key

- **WHEN** a developer runs `falabella-installer config set registry.url https://skills.falabella.internal` then `falabella-installer config get registry.url`
- **THEN** the second command prints `https://skills.falabella.internal` and exits zero

#### Scenario: Reject unknown key

- **WHEN** a developer runs `falabella-installer config set bogus.key value`
...
- **WHEN** a developer runs `falabella-installer install code-review-standard --json`
...
- **WHEN** a developer runs `falabella-installer list --json`
...
- **WHEN** a developer runs `falabella-installer install <skill>` on a host with the registry available but all other outbound traffic blocked
