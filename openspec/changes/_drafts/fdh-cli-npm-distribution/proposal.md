...
1. **`npx @forge/fdh init` funciona sin install previo.** Una sola línea, sin tap, sin source winget, sin tarball, sin PATH editing manual. El primer dev que prueba FDH no tiene fricción.
...
- **Scope npm**: `@forge/` para distinguir de paquetes públicos. Configurable vía `.npmrc` corporativo que apunta `@forge:registry=https://artifactory.forge.internal/api/npm/npm-internal/`.
...
- **Primary**: `npx @forge/fdh init` (zero-install) y `npm i -g @forge/fdh` (persistent). Documentación de quickstart lidera con esto.
...
- `README.md` del hub: lead con `npx @forge/fdh init` como ejemplo primario.
...
- `fdh-npm-wrapper`: el contrato del paquete `@forge/fdh` — qué hace el wrapper, cómo descarga el binario, cómo lo verifica, cómo propaga args y exit codes, qué pasa si la descarga falla, comportamiento offline (cache hit), versionado.
...
- Camino de upgrade suave: el próximo release publica al npm registry. Los pilot devs pueden hacer `npm i -g @forge/fdh` y reemplazar el binario instalado vía tarball/brew sin perder config (`~/.config/fdh/config.yaml` se mantiene).
...
- **Opción 2 — repo separado `@forge/fdh-npm`**: separación limpia de toolchain. Versión sincronizada a mano (o vía bot). Más overhead organizacional.

Recomendación tentativa: Opción 1 (sub-directorio), siguiendo el pattern de esbuild y biome.

### Sets up future changes

- `fdh-upgrade-command`: `fdh upgrade` puede ahora simplemente `npm update -g @forge/fdh`, sin lógica custom de auto-update. Mucho más simple que en el modelo brew/tarball.
...
- ¿El binario alias `fdh` se shipea desde el mismo paquete o desde un `@forge/fdh-stub` separado?
