...
El plan archivado `add-fdh-cli-distribution-and-interactive-init` definió tres canales paralelos de distribución del binario `fdh` (Go): one-liner `install.sh`/`install.ps1`, tap interno de Homebrew, paquete winget interno. La Decision 6 reconoce que firmar los binarios requiere proceso corporativo Falabella (Authenticode + macOS notarization) que tarda **semanas a meses**, y propone degradación con warning como Day 1 aceptable.
...
1. **`npx @falabella/fdh init` funciona sin install previo.** Una sola línea, sin tap, sin source winget, sin tarball, sin PATH editing manual. El primer dev que prueba FDH no tiene fricción.
...
La premisa que habilita el cambio: **la mayoría de los devs Falabella ya tienen Node instalado** porque sus herramientas IA lo requieren (Claude Code corre en Node, VS Code embebe Node, frontend toolchain entero es Node). El "requiere Node" deja de ser una barrera real en 2026.
...
- **Provisionar JFrog Artifactory Self-Hosted (Pro)** como registry interno Falabella. Decisión basada en:
  ...
  - **GitLab Package Registry** como tercera opción si Falabella ya tiene GitLab Enterprise (incluido sin costo extra).
- **Scope npm**: `@falabella/` para distinguir de paquetes públicos. Configurable vía `.npmrc` corporativo que apunta `@falabella:registry=https://artifactory.falabella.internal/api/npm/npm-internal/`.
...
- **Primary**: `npx @falabella/fdh init` (zero-install) y `npm i -g @falabella/fdh` (persistent). Documentación de quickstart lidera con esto.
...
### Back-compat del nombre `falabella-installer`

- El stub `cmd/falabella-installer-stub` del repo `fdh` (introducido por `dev-portal` para 90 días de deprecation) se mantiene tal cual.
- Adicionalmente: el paquete npm puede shipear un binario alias `falabella-installer` que también apunta al wrapper, por simetría. A confirmar en design.md.

### Documentación

- `README.md` del hub: lead con `npx @falabella/fdh init` como ejemplo primario.
...
- `fdh-npm-wrapper`: el contrato del paquete `@falabella/fdh` — qué hace el wrapper, cómo descarga el binario, cómo lo verifica, cómo propaga args y exit codes, qué pasa si la descarga falla, comportamiento offline (cache hit), versionado.
...
- Coordinación con seguridad: la URL de descarga del binario (`${FDH_PKG_HOST}/fdh/<version>/...`) debe ser whitelisteada en proxies corporativos para que el postinstall funcione desde laptops detrás del firewall.

### Pilot devs existentes

- Camino de upgrade suave: el próximo release publica al npm registry. Los pilot devs pueden hacer `npm i -g @falabella/fdh` y reemplazar el binario instalado vía tarball/brew sin perder config (`~/.config/fdh/config.yaml` se mantiene).
...
- **Opción 2 — repo separado `@falabella/fdh-npm`**: separación limpia de toolchain. Versión sincronizada a mano (o vía bot). Más overhead organizacional.

Recomendación tentativa: Opción 1 (sub-directorio), siguiendo el pattern de esbuild y biome.

### Sets up future changes

- `fdh-upgrade-command`: `fdh upgrade` puede ahora simplemente `npm update -g @falabella/fdh`, sin lógica custom de auto-update. Mucho más simple que en el modelo brew/tarball.
...
- `multi-platform-binary-tests`: el wrapper TS habilita tests cross-platform vía GitHub Actions matrix sin necesitar runners self-hosted Falabella.
...
- ¿El binario alias `falabella-installer` se shipea desde el mismo paquete o desde un `@falabella/falabella-installer-stub` separado?
