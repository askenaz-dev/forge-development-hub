...
- [ ] 1.1 Confirmar con plataforma Falabella si JFrog Artifactory Pro está disponible o si se aprueba provisioning <!-- pending: external (platform team) -->
...
- [ ] 1.3 Provisionar el registry elegido con un repo npm virtual `npm-internal` y scope `@falabella/` <!-- pending: external (platform team) -->
- [x] 1.4 Crear `.npmrc` template corporativo documentando `@falabella:registry=https://<registry-host>/api/npm/npm-internal/` + autenticación (token corporativo) <!-- shipped as C:/falabella/fdh/npm/.npmrc.template -->
...
- [ ] 2.2 Validar que devs corporativos pueden hacer `npm install` desde un paquete @falabella sin warnings de cert <!-- pending: depends on 1.x + 2.1 -->
- [x] 2.3 Documentar troubleshooting en `docs/troubleshooting.md` del repo `fdh` para casos de cert inspection (NODE_EXTRA_CA_CERTS, NPM_CONFIG_CAFILE) <!-- shipped as C:/falabella/fdh/docs/troubleshooting.md -->

## 3. Sub-directorio `npm/` en el repo `fdh`

- [x] 3.1 Crear `npm/` en `C:/falabella/fdh/` (o donde corresponda) con `package.json`, `tsconfig.json`, `src/`, `scripts/`, `dist/` (gitignored) <!-- C:/falabella/fdh/npm/, npm/dist/ added to fdh .gitignore -->
- [x] 3.2 Configurar `package.json` con `name: "@falabella/fdh"`, `bin: { fdh: "./dist/cli.js", falabella-installer: "./dist/cli.js" }`, `scripts: { build, test, postinstall }`, `engines: { node: ">=18" }` <!-- bin maps to separate cli.js + cli-alias.js for cleaner alias dispatch -->
...
- [x] 4.1 Implementar `npm/src/index.ts` (wrapper CLI): detecta `process.platform`/`process.arch`, resuelve path del binario en `node_modules/@falabella/fdh/bin/fdh<.exe>`, hace `spawn` con `stdio: 'inherit'`, propaga exit code <!-- shipped as src/cli.ts; propagates signals too -->
- [x] 4.2 Implementar manejo del alias `falabella-installer`: detecta nombre del invocador via `process.argv[1]`, si es `falabella-installer` imprime warning de deprecation + delega a `fdh` <!-- shipped as separate src/cli-alias.ts bin entry, cleaner cross-platform than argv[1] detection -->
- [x] 4.3 Implementar fallback informativo si el binario no existe: "fdh: binary not found at <path>; run 'npm rebuild @falabella/fdh' to repair" <!-- verified locally -->
...
- [x] 5.4 Implementar extracción del tarball a `node_modules/@falabella/fdh/bin/` con permisos correctos (chmod +x en Unix) <!-- shells out to `tar` (built-in on Win 10+, mac, linux) -->
...
- [ ] 6.1 Probar instalación vía `npm i @falabella/fdh` (smoke local) <!-- pending: needs published package or local pack/tarball test against running registry -->
- [ ] 6.2 Probar instalación vía `pnpm add @falabella/fdh` (smoke local, verificar layout symlinked) <!-- pending: same -->
- [ ] 6.3 Probar instalación vía `yarn add @falabella/fdh` (yarn classic y berry si aplica) <!-- pending: same -->
...
- [x] 7.1 Extender `.github/workflows/release.yml` (o equivalente Falabella) para que un `git tag vX.Y.Z` dispare: (a) cross-compile Go para 5 targets, (b) upload binarios + SHA-256 + manifest a `${FDH_PKG_HOST}`, (c) `cd npm && npm version <X.Y.Z> --no-git-tag-version && npm run build && npm publish` <!-- new `publish-npm` job after the existing `publish` job, with --allow-same-version idempotency -->
...
- [x] 9.1 Actualizar `README.md` del hub para liderar con `npx @falabella/fdh init` <!-- shipped as falabella-development-hub/README.md (new file) -->
- [x] 9.2 Reescribir `docs/quickstart.md` del repo `fdh`: npm first, brew/install.sh second, tarball manual last <!-- C:/falabella/fdh/docs/quickstart.md fully rewritten -->
- [ ] 9.3 Actualizar el portal web (`/install`) para que CTA principal sea npm; OS-detected commands liderar con `npx`/`npm i -g` <!-- pending: separate effort in C:/falabella/fdh/web/ (Next.js app); tracked as follow-up since portal-web spec changes may be needed -->
- [x] 9.4 Crear `docs/troubleshooting.md` con secciones: proxies corporativos, cert inspection, package manager edge cases, cache miss <!-- C:/falabella/fdh/docs/troubleshooting.md -->
- [ ] 9.5 Comunicación interna del rollout: anuncio mailing list + Slack con `npx @falabella/fdh init` como hook de adopción <!-- pending: external (comms/dx-platform) -->

## 10. Pilot upgrade path

- [ ] 10.1 Probar upgrade de un pilot dev: `brew uninstall fdh && npm i -g @falabella/fdh && fdh --version` mantiene config (`~/.config/fdh/config.yaml`) <!-- pending: needs real npm registry + pilot machine -->
- [ ] 10.2 Probar upgrade desde tarball install: dev borra el binario manual, `npm i -g @falabella/fdh`, comprueba `which fdh` apunta al de npm <!-- pending: same -->
...
- [ ] 11.1 Smoke test end-to-end: `npx @falabella/fdh init` en máquina sin install previo de fdh, completa exitosamente, verifica que `fdh doctor` reporta OK <!-- pending: needs published package -->
...
- [x] 11.4 Verificar que el alias `falabella-installer` imprime deprecation warning y delega correctamente <!-- verified locally: `node dist/cli-alias.js --version` prints warning + delegates to runFdh; 41/41 unit tests pass -->
- [ ] 11.5 Verificar versionado 1:1: `npm view @falabella/fdh version` == `fdh --version` <!-- pending: needs first published release -->
