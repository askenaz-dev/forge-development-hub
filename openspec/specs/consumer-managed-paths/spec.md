# consumer-managed-paths Specification

## Purpose

Establece el contrato que separa lo que FDH posee de lo que el developer posee dentro de un consumer repo: marker `.fdh-managed.yaml` por componente instalado (con `name`, `kind`, `version`, `hub_commit`, `installed_at`, `installed_by_fdh`, `source_path`), `.gitignore` sectionado e idempotente entre markers `# >>> fdh:managed-paths >>>` / `# <<< fdh:managed-paths <<<` que FDH modifica sin tocar el resto, y bloque managed dentro de `.claude/settings.json` para hooks con marker `_fdh_managed: <name>` para uninstall selectivo. La ausencia del marker propio significa intocable —los componentes handwritten del developer coexisten con los managed sin riesgo de stomp.
