# hub-hooks-primitive Specification

## Purpose

Introduce `hook` como primitiva del hub, materializada como directorios `hooks/<name>/` que contienen `HOOK.md` (documentación humana) + `hook.json` (configuración ejecutable con `event`, `matcher`, `command`, opcionalmente `profile`/`timeout_seconds`/`description`). FDH no implementa runtime de hooks propio: materializa la configuración como bloque managed dentro de `.claude/settings.json` (con marker `_fdh_managed: <name>` para uninstall selectivo) preservando entries pre-existentes del developer, y delega la ejecución del comando al agente target. Ship con la primera entry real `hooks/doctor-on-session-start/` que dispara `fdh doctor --quiet` en `SessionStart`.
