...
Wrappers de package manager + scripts install per-OS + manifest publisher + endpoint configurable vía FDH_PKG_HOST. Cierra el gap entre el binario fdh y el PATH del developer.
...
- **WHEN** el manifest declara `signed: true` para la versión instalada
...
#### Scenario: Binario no firmado

- **WHEN** el manifest declara `signed: false`
- **THEN** el instalador imprime un warning nombrando explícitamente que el binario no está firmado, indica que el SHA-256 fue verificado contra el manifest, sugiere postergar instalación si la política de seguridad del developer requiere firma, y continúa con código cero
