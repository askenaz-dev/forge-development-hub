...
Define el contrato del paquete `@falabella/fdh` publicado al registry npm interno: shipea el binario Go vía un wrapper TypeScript de zero runtime deps, descarga el binario correcto para el OS/arch del host en `postinstall` síncrono con verificación SHA-256, honra proxies corporativos en cascada (npm_config → env vars → NO_PROXY), soporta los tres principales package managers (npm/pnpm/yarn) incluyendo cache hit offline, propaga args/exit code sin transformación, y mantiene el alias backwards-compatible `falabella-installer` con warning de deprecation. La versión del paquete npm SHALL ser 1:1 con la del binario Go (sin cycles independientes).

## Requirements

### Requirement: Paquete `@falabella/fdh` distribuye el binario Go vía npm

El sistema SHALL publicar un paquete npm `@falabella/fdh` al registry interno (JFrog Artifactory Pro o fallback aprobado) cuyo contenido es un wrapper TypeScript + un script postinstall que descarga el binario `fdh` Go correcto para el OS/arch del host. El paquete SHALL declarar `bin: { fdh, falabella-installer }` apuntando al mismo wrapper, para que ambos nombres queden disponibles en PATH tras `npm i -g` o vía `npx`.

#### Scenario: Install global con npm

- **WHEN** un developer ejecuta `npm i -g @falabella/fdh`
- **THEN** después del comando, los binarios `fdh` y `falabella-installer` están en PATH y `fdh --version` responde con la versión del paquete instalado

#### Scenario: Zero-install con npx

- **WHEN** un developer ejecuta `npx @falabella/fdh init` sin install previo
...
- **WHEN** se publica `@falabella/fdh@0.7.2` al registry interno
...
El script `postinstall` SHALL ejecutar sincrónico (bloqueando `npm install`/`npx`/`pnpm add`/`yarn add`) y SHALL: (a) detectar `process.platform` y `process.arch` para resolver el target (`darwin-arm64`, `darwin-amd64`, `linux-arm64`, `linux-amd64`, `windows-amd64`), (b) descargar `${FDH_PKG_HOST}/fdh/<version>/fdh-<target>.tar.gz` y `<...>.sha256`, (c) verificar SHA-256, (d) extraer a `node_modules/@falabella/fdh/bin/`. Si cualquier paso falla, el postinstall SHALL fallar con código distinto de cero y mensaje accionable.

#### Scenario: Postinstall exitoso

- **WHEN** un developer corre `npm i @falabella/fdh` en darwin-arm64 con conectividad al registry
...
- **WHEN** el developer corre `npm i @falabella/fdh` con `npm_config_https_proxy=http://corp-proxy:8080` heredado
- **THEN** el postinstall enruta la descarga del binario por ese proxy

#### Scenario: NO_PROXY excluye el host interno

- **WHEN** `NO_PROXY=*.falabella.internal` y `${FDH_PKG_HOST}=artifactory.falabella.internal/...`
...
- **WHEN** un developer ejecuta `pnpm add -g @falabella/fdh`
...
- **WHEN** un developer ejecuta `yarn global add @falabella/fdh`
...
Si el binario ya está extraído en `node_modules/@falabella/fdh/bin/` con un SHA-256 que coincide con el esperado para la versión instalada, el postinstall SHALL detectarlo y SHALL saltar la descarga (cache hit). Esto permite re-ejecuciones offline después del primer install exitoso.

#### Scenario: Cache hit en re-install

- **WHEN** el binario ya existe con SHA-256 correcto y se re-ejecuta `npm i @falabella/fdh`
...
- **THEN** el wrapper imprime "fdh: binary not found at <path>; run 'npm rebuild @falabella/fdh' to repair" y sale con código distinto de cero

### Requirement: Documentación de instalación lidera con npm

La documentación oficial (`README.md` del hub, `docs/quickstart.md` del repo `fdh`, portal web `/install`) SHALL presentar `npx @falabella/fdh init` como comando primario para el primer install, con `npm i -g @falabella/fdh` como alternativa persistente. Otros canales (brew/install.sh/.deb/.rpm) SHALL aparecer como alternativas en secciones secundarias.

#### Scenario: README del hub

- **WHEN** un nuevo dev lee `README.md` del hub
- **THEN** encuentra `npx @falabella/fdh init` como ejemplo primario en la sección de instalación, antes de mencionar brew/tarball

#### Scenario: Portal `/install` por defecto

- **WHEN** un usuario sin OS detectado abre `/install` en el portal web
- **THEN** ve el comando `npx @falabella/fdh init` como CTA principal, con tabs/expandables para alternativas

### Requirement: Backwards compat con `falabella-installer`

El paquete SHALL declarar el binario alias `falabella-installer` que ejecuta el mismo wrapper que `fdh`, manteniendo compat con docs y scripts que aún referencien el nombre viejo durante la ventana de deprecation de 90 días definida en el change `dev-portal`. El comando alias SHALL imprimir un one-liner informativo de deprecation y delegar a `fdh`.

#### Scenario: Invocación del alias

- **WHEN** un script existente ejecuta `falabella-installer doctor`
- **THEN** el wrapper imprime "falabella-installer is deprecated; please use 'fdh' instead (alias will be removed YYYY-MM-DD)" y luego ejecuta `fdh doctor` propagando el exit code
