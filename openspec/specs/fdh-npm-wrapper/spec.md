...
Define el contrato del paquete `@forge/fdh` publicado al registry npm interno: shipea el binario Go vía un wrapper TypeScript de zero runtime deps, descarga el binario correcto para el OS/arch del host en `postinstall` síncrono con verificación SHA-256, honra proxies corporativos en cascada (npm_config → env vars → NO_PROXY), soporta los tres principales package managers (npm/pnpm/yarn) incluyendo cache hit offline, y propaga args/exit code sin transformación. La versión del paquete npm SHALL ser 1:1 con la del binario Go (sin cycles independientes). Ningún binario alias `forge-installer` ni paquete `@forge/fdh` se publica.
## Requirements
