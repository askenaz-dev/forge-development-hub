...
- **WHEN** a reviewer diffs `references/AGENTS.md` against the source at `FTI00575-design-system/AGENTS.md` at the pinned `.ds-version`
...
- **THEN** it MUST contain three lines (or three keys in a structured format) identifying: upstream commit SHA, UI package semver, and sync date in ISO 8601
...
`scripts/sync.mjs` SHALL accept a path to a local clone of `FTI00575-design-system` (via `--source <path>` flag with sensible default such as `../FTI00575-design-system`), copy the required files into `references/`, and rewrite `.ds-version` with the SHA, package version, and current ISO date. It MUST NOT clone, fetch, or perform any network operation. Running it twice in a row against the same source MUST produce identical output (idempotent).
...
- **WHEN** a reviewer reads `SKILL.md`
- **THEN** the file MUST contain a "Setup" section with all four sub-items: `.npmrc` config, PAT instructions, CLI usage, npm package alternative
