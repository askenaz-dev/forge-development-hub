...
Define el ledger per-máquina `~/.fdh/state.json` (no committed, vive en HOME) que registra el inventario de instalaciones del usuario: user-scope installs por kind, cache del hub, y una sección opcional `projects:` de bajo costo que se llena bajo demanda al correr `fdh install` en cada proyecto. Habilita los comandos `fdh list-installed`, `fdh repair` y `fdh uninstall --dry-run`, completando el patrón npm/yarn/pnpm de "manifest + lock + cache global" con la pieza que faltaba.
