# hub-rules-primitive Specification

## Purpose

Introduce `rule` como primitiva del hub distinta de `skill`, materializada como directorios `rules/<name>/RULE.md` con frontmatter normativo (`name`, `kind: rule`, `scope` por glob, `severity`, `agents_supported`). Define la materialización per-agente —`.claude/rules/<name>.md`, `.codex/rules/<name>.md`, `.github/rules/<name>.md`, `.opencode/rules/<name>.md`— preservando el cuerpo markdown y transformando frontmatter al formato esperado por cada ecosistema. Ship con la primera entry real `rules/no-console-log/`, validando el patrón end-to-end y dejando camino hecho para futuros `add-more-rules`.
