# hub-registry-v2 Specification

## Purpose

Define el catálogo unificado del hub en su segunda versión de schema: `schema_version: 2` con campo obligatorio `kind` (`skill | rule | agent | hook`), validación de coherencia entre kind y directorio raíz (`skills/`, `rules/`, `agents/`, `hooks/`), detección de directorios huérfanos, y migración tooled e idempotente desde el schema v1. Esta capability fija las reglas de evolución del catálogo a medida que crece más allá de la primera entrada `design-system`, antes de que el costo de migrar entradas legacy sea prohibitivo.
